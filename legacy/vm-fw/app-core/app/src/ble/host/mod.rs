...
mod icmsg_config {
    ...
    pub fn get_icmsg_config() -> icmsg::MemoryConfig {
        unsafe {
            let send_buffer_len = (&raw const __icmsg_tx_end).byte_offset_from(&raw const __icmsg_tx_start) as u32
                - size_of::<icmsg::transport::SharedMemoryRegionHeader<ALIGN>>() as u32;
            let recv_buffer_len = (&raw const __icmsg_rx_end).byte_offset_from(&raw const __icmsg_rx_start) as u32
                - size_of::<icmsg::transport::SharedMemoryRegionHeader<ALIGN>>() as u32;
            icmsg::MemoryConfig {
...
pub type BleController = ExternalController<MyTransport<NoopRawMutex, IpcWait<'static>, IpcNotify<'static>>, 10>;
...
pub async fn init(mut ipc: Ipc<'static>) -> BleController {
    ...
    defmt::trace!("{:?}", Debug2Format(&icmsg_config));
    let icmsg = unsafe {
    ...
    let icmsg = match icmsg {
        Err(e) => {
            defmt::panic!("Failed to initialize IcMsg: {:?}", Debug2Format(&e));
        }
        Ok(icmsg) => {
